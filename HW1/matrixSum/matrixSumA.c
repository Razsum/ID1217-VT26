...
struct matrixElement matrixMin[MAXWORKERS];
struct matrixElement matrixMax[MAXWORKERS];
...
void *Worker(void *arg) {
  ...
  if (myid == 0) {
    ...
    for (i = 0; i < numWorkers; i++){
      total += sums[i];
      if (min.value > matrixMin[i].value)
        min = matrixMin[i];
      if (max.value < matrixMax[i].value)
        max = matrixMax[i];
    }
    /* get end time */
    end_time = read_timer();
