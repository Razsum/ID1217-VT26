...
void *Worker(void *arg) {
  long myid = (long) arg;
  int i, j, first, last, threadTotal;
  ...
  threadMax.value = matrix[first][0];
  threadMax.row = first;
  threadMax.col = 0;
  threadMin.value = matrix[first][0];
  threadMin.row = first;
  threadMin.col = 0;
  threadTotal = 0;
  ...
  for (i = first; i <= last; i++){
    for (j = 0; j < size; j++){
      if (threadMax.value < matrix[i][j]) {
        threadMax.value = matrix[i][j];
        threadMax.row = i;
        threadMax.col = j;
      } else if(threadMin.value > matrix[i][j]) {
        threadMin.value = matrix[i][j];
        threadMin.row = i;
        threadMin.col = j;
      }
      threadTotal += matrix[i][j];
    }
  }
